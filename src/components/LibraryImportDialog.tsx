  ...
  const importMutation = useMutation({
    mutationFn: async () => {
      ...
      const formattedData = previewData.map(row => {
        const copies = parseInt(row.total_copies) || 1;
        return {
          title: row.title || "Untitled",
          author: row.author || null,
          isbn: row.isbn?.toString() || null,
          total_copies: copies,
          available_copies: copies,
        };
      });

      const { error } = await supabase.from('books').insert(formattedData);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['books'] });
      toast.success(`Successfully imported ${previewData.length} books!`);
      resetState();
    },
    onError: (err: any) => {
    ...
    <Dialog open={open} onOpenChange={(val) => {
      ...
      <DialogTrigger asChild>
        <Button variant="outline" className="gap-2">
          <Upload className="h-4 w-4" /> Import Books
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            Upload an Excel (.xlsx) or CSV file to bulk import books into the library.
          </DialogDescription>
        </DialogHeader>
                ...
                <table className="w-full text-sm text-left">
                  <thead className="bg-muted">
                    <tr>
                      ...
                      <th className="p-2 font-medium">ISBN</th>
                      <th className="p-2 font-medium">Total Copies</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y">
                    {previewData.slice(0, 3).map((row, idx) => (
                      <tr key={idx}>
                        ...
                        <td className="p-2">{row.total_copies || "—"}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
